...
const ProductDetails = () => {
	...
	useEffect(()=>{
        fetch(`https://arcane-stream-89038.herokuapp.com/readUserData?email=${user?.email}`)
		.then(res=>res.json())
	...
	const { isLoading: productLoading, data: product } = useQuery('productdata', () =>
		fetch(`https://arcane-stream-89038.herokuapp.com/readSingleToolsData/${id}`).then(res =>
			res.json()
		)
	)
	const phone = useRef('');
	...
	const handleFormsubmit = (e) => {
        ...
        e.preventDefault();
        fetch("https://arcane-stream-89038.herokuapp.com/addOrder", {
            method: "POST",
            headers: {
